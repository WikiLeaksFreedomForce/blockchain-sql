...
int32_t read_blk(uint8_t** datap, blk_t* blk);
...
int32_t read_blk(uint8_t** datap, blk_t* blk)
{
        ...
        if (blk->blk_magic != 0xD9B4BEF9)
        {
                *datap -= 3; //rewind three bytes so we can call this func again 1 byte advanced
                //this will slide the buffer pointer along until it either reaches a valid block
                //or the file pointer check determines we're at the end of the file.
                if (!skipblock)
                {
                        skipblock = 1;
                        printf("\nInvalid block magic. Seeking...\n");
                }
                return -1;
        }
        skipblock = 0;
...
int32_t sql_prepare()
{
        ...
        snprintf(query_buf, 1024 * 1024 * 2, "INSERT OR IGNORE INTO BLOCK (ID,PREV,MERKLE,HASH,BITS,NONCE,TIME) VALUES (?,?,?,?,?,?,?)");
        if (sqlite3_prepare_v2(db, query_buf, -1, &blk_insert_stmt, 0) != SQLITE_OK)
                return -1;
        memset(query_buf, 0, 1024 * 1024 * 2);

        snprintf(query_buf, 1024 * 1024 * 2, "INSERT OR IGNORE INTO FILE (ID,NAME) VALUES (?,?)");
        if (sqlite3_prepare_v2(db, query_buf, -1, &file_insert_stmt, 0) != SQLITE_OK)
                return -1;
        memset(query_buf, 0, 1024 * 1024 * 2);

        snprintf(query_buf, 1024 * 1024 * 2, "INSERT OR IGNORE INTO TX (ID,HASH,FILE,OFFSET) VALUES (?,?,?,?)");
        if (sqlite3_prepare_v2(db, query_buf, -1, &tx_insert_stmt, 0) != SQLITE_OK)
                return -1;
        memset(query_buf, 0, 1024 * 1024 * 2);

        snprintf(query_buf, 1024 * 1024 * 2, "INSERT OR IGNORE INTO REL_BLOCK_TX (ID,BLOCK,TX) VALUES (?,?,?)");
        if (sqlite3_prepare_v2(db, query_buf, -1, &rel_blk_insert_stmt, 0) != SQLITE_OK)
                return -1;
        memset(query_buf, 0, 1024 * 1024 * 2);

        snprintf(query_buf, 1024 * 1024 * 2, "INSERT OR IGNORE INTO TX_INPUT (ID,TX,PREV,IDX,SEQ,DATA) VALUES (?,?,?,?,?,?)");
        if (sqlite3_prepare_v2(db, query_buf, -1, &tx_input_insert_stmt, 0) != SQLITE_OK)
                return -1;
        memset(query_buf, 0, 1024 * 1024 * 2);

        snprintf(query_buf, 1024 * 1024 * 2, "INSERT OR IGNORE INTO REL_TX_INPUT (ID,TX,INPUT) VALUES (?,?,?)");
        if (sqlite3_prepare_v2(db, query_buf, -1, &rel_tx_input_insert_stmt, 0) != SQLITE_OK)
                return -1;
        memset(query_buf, 0, 1024 * 1024 * 2);

        snprintf(query_buf, 1024 * 1024 * 2, "INSERT OR IGNORE INTO TX_OUTPUT (ID,TX,VAL,DATA,ADDR) VALUES (?,?,?,?,?)");
        if (sqlite3_prepare_v2(db, query_buf, -1, &tx_output_insert_stmt, 0) != SQLITE_OK)
                return -1;
        memset(query_buf, 0, 1024 * 1024 * 2);

        snprintf(query_buf, 1024 * 1024 * 2, "INSERT OR IGNORE INTO REL_TX_OUTPUT (ID,TX,OUTPUT) VALUES (?,?,?)");
...
int32_t sql_insert_block(uint32_t id, char prev[65], char merkle[65], char hash[65], uint32_t bits, uint32_t nonce, uint32_t time)
{
        ...
        if (sqlite3_bind_text(blk_insert_stmt, 2, prev, -1, SQLITE_STATIC) < 0) return -1;
        if (sqlite3_bind_text(blk_insert_stmt, 3, merkle, -1, SQLITE_STATIC) < 0) return -1;
        if (sqlite3_bind_text(blk_insert_stmt, 4, hash, -1, SQLITE_STATIC) < 0) return -1;
...
int32_t sql_insert_file(uint32_t id, const char* file)
{
        ...
        if (sqlite3_bind_text(file_insert_stmt, 2, file, -1, SQLITE_STATIC) < 0) return -1;
...
int32_t sql_insert_tx(uint32_t id, char hash[65], uint32_t file, uint32_t offset)
{
        ...
        if (sqlite3_bind_text(tx_insert_stmt, 2, hash, -1, SQLITE_STATIC) < 0) return -1;
...
int32_t sql_insert_tx_input(uint32_t id, uint32_t tx, char prev[65], uint32_t idx, uint32_t seq, uint8_t* data, uint64_t datalen)
{
        ...
        if (sqlite3_bind_text(tx_input_insert_stmt, 3, prev, -1, SQLITE_STATIC) < 0) return -1;
        if (sqlite3_bind_int(tx_input_insert_stmt, 4, idx) < 0) return -1;
        if (sqlite3_bind_int(tx_input_insert_stmt, 5, seq) < 0) return -1;
        if (sqlite3_bind_blob(tx_input_insert_stmt, 6, data, datalen, SQLITE_STATIC) < 0) return -1;
...
int32_t sql_insert_tx_output(uint32_t id, uint32_t tx, uint64_t val, uint8_t* data, uint64_t datalen, char* addr)
{
        ...
        if (sqlite3_bind_blob(tx_output_insert_stmt, 4, data, datalen, SQLITE_STATIC) < 0) return -1;
        if (sqlite3_bind_text(tx_output_insert_stmt, 5, addr, -1, SQLITE_STATIC) < 0) return -1;
...
int main(int argc, char** argv)
{
        ...
        create1 = "CREATE TABLE IF NOT EXISTS BLOCK("  \
        ...
        create2 = "CREATE TABLE IF NOT EXISTS TX(" \
                "ID INT PRIMARY KEY             NOT NULL," \
                "HASH           CHAR(64)        NOT NULL," \
                "FILE           TEXT            NOT NULL," \
                "OFFSET         INT             NOT NULL);";

        create3 = "CREATE TABLE IF NOT EXISTS TX_INPUT(" \
                "ID INT PRIMARY KEY             NOT NULL," \
                "TX             INT             NOT NULL," \
                "PREV           CHAR(64)        NOT NULL," \
                "DATA           BLOB            NOT NULL," \
        ...
        create4 = "CREATE TABLE IF NOT EXISTS TX_OUTPUT(" \
                "ID INT PRIMARY KEY             NOT NULL," \
                "TX             INT             NOT NULL," \
                "DATA           BLOB            NOT NULL," \
                "ADDR           TEXT            ," \
        ...
        create5 = "CREATE TABLE IF NOT EXISTS FILE(" \
                "ID INT PRIMARY KEY             NOT NULL," \
                "NAME           FILE            NOT NULL);";

        create6 = "CREATE TABLE IF NOT EXISTS REL_BLOCK_TX(" \
                "ID INT PRIMARY KEY             NOT NULL," \
                "BLOCK          INT             NOT NULL," \
                "TX             INT             NOT NULL);";

        create7 = "CREATE TABLE IF NOT EXISTS REL_TX_INPUT(" \
                "ID INT PRIMARY KEY             NOT NULL," \
                "TX             INT             NOT NULL," \
                "INPUT          INT             NOT NULL);";

        create8 = "CREATE TABLE IF NOT EXISTS REL_TX_OUTPUT(" \
                "ID INT PRIMARY KEY             NOT NULL," \
                "TX             INT             NOT NULL," \
                "OUTPUT         INT             NOT NULL);";

        create9 = "CREATE TABLE IF NOT EXISTS REL_BLOCK_NEXT(" \
                "ID INT PRIMARY KEY             NOT NULL," \
                "BLOCK          INT             NOT NULL," \
                "NEXT           INT             NOT NULL);";

        create10 = "CREATE TABLE IF NOT EXISTS REL_BLOCK_PREV(" \
                "ID INT PRIMARY KEY             NOT NULL," \
                "BLOCK          INT             NOT NULL," \
                "PREV           INT             NOT NULL);";

        printf("Creating database...\n");
        if (sql_query(create1) < 0) { printf("create1 error.\n");  return -1; }
        if (sql_query(create2) < 0) { printf("create2 error.\n");  return -1; }
        if (sql_query(create3) < 0) { printf("create3 error.\n");  return -1; }
        if (sql_query(create4) < 0) { printf("create4 error.\n");  return -1; }
        if (sql_query(create5) < 0) { printf("create5 error.\n");  return -1; }
        if (sql_query(create6) < 0) { printf("create6 error.\n");  return -1; }
        if (sql_query(create7) < 0) { printf("create7 error.\n");  return -1; }
        if (sql_query(create8) < 0) { printf("create8 error.\n");  return -1; }
        if (sql_query(create9) < 0) { printf("create9 error.\n");  return -1; }
        if (sql_query(create10) < 0) { printf("create10 error.\n");  return -1; }
        ...
        else if (len >= 25) //format 3,4,5
        {
                ...
                for (i = 0; i<len; i++)
                {
                        if (data[i] == 0xA9) //OP_HASH160
                        {
                                ...
                                {
                                        #ifdef __WIN32__
                                        strcpy_s(ret, 128, "STRANGE");
                                        #else
                                        strlcpy(ret,"STRANGE",128);
                                        #endif
                                        return ret;
                                }
                        }
...
{
        #ifdef __WIN32__
        int32_t fd = _open(filename, O_RDONLY | _O_BINARY);
        #else
        ...
        while (rbytes < file_size)
        {
                ...
                #ifdef __WIN32__
                int32_t n = _read(fd, file_data + rbytes, file_size - rbytes);
                #else
        ...
        while (filep != (file_data + file_size))
        {
                ...
                float cur_perc = (float)((float)(filep - file_data) / (float)file_size) * 100.0f;
                if (cur_perc - last_perc > 0.2f)
                {
                        printf("blk %u tx %u (%u/%u) %s (%.2f%%)     \n", blk_cnt, tx_cnt, txin_cnt, txout_cnt, filename, cur_perc);
                        fflush(stdout);
                        last_perc = cur_perc;
                }
                free_blk(blk);
